...
_F1_CANDIDATE_KEYS = (
    ...
    'Frame F1',
    'Onset + velocity F1',
    'Onset + offset + velocity F1',
)


def _flatten_metric_mapping(
    metrics: Optional[Mapping[str, object]], prefix: str = ''
) -> Dict[str, object]:
  """Flattens nested metric mappings into slash-separated keys."""
  ...
  for key, value in metrics.items():
    ...
    if isinstance(value, Mapping):
      flat.update(_flatten_metric_mapping(value, full_key))
    else:
      flat[full_key] = value
  return flat


def _metric_value_to_float(value: object) -> Optional[float]:
  """Normalizes scalar metric values into Python floats."""
  if value is None:
    return None
...
def _select_metric_value(
    metrics: Mapping[str, object], candidate_keys: Sequence[str]
) -> Tuple[Optional[str], Optional[float]]:
  """Returns the best matching metric name/value pair from a flat mapping."""
  lowered = {key.lower(): key for key in metrics.keys()}
  for candidate in candidate_keys:
    candidate_lower = candidate.lower()
    for key_lower, original_key in lowered.items():
      if key_lower == candidate_lower or key_lower.endswith(f'/{candidate_lower}'):
        value = _metric_value_to_float(metrics[original_key])
        if value is not None:
          return original_key, value
  for original_key, value in metrics.items():
    key_lower = original_key.lower()
    if 'f1' in key_lower and 'hist' not in key_lower:
      numeric_value = _metric_value_to_float(value)
      if numeric_value is not None:
        return original_key, numeric_value
  return None, None
...
def _build_progress_postfix(
    ...
    f1_name: Optional[str],
) -> Dict[str, str]:
  """Builds a compact tqdm postfix dictionary."""
  postfix = {
      'loss': f'{loss_value:.4f}' if loss_value is not None else 'n/a',
      'f1': f'{f1_value:.4f}' if f1_value is not None else 'n/a',
  }
  if f1_name:
    postfix['f1_metric'] = f1_name.split('/')[-1]
  return postfix


def _create_epoch_progress_bar(
    epoch_index: int,
    epoch_count: int,
    total_steps: int,
):
  """Creates a tqdm progress bar on host 0 when available."""
  if tqdm is None or jax.process_index() != 0 or not sys.stderr.isatty():
    return None
  return tqdm(
      total=total_steps,
      desc=f'Epoch {epoch_index}/{epoch_count}',
      unit='step',
      dynamic_ncols=True,
      leave=True,
  )
...
def train(
  ...
  def _update_last_f1(metrics: Optional[Mapping[str, object]]) -> None:
    nonlocal last_f1_name
    nonlocal last_f1_value
    flat_metrics = _flatten_metric_mapping(metrics)
    metric_name, metric_value = _select_metric_value(
        flat_metrics, _F1_CANDIDATE_KEYS
    )
    if metric_value is not None:
      last_f1_name = metric_name
      last_f1_value = metric_value
  ...
  def _run_training_eval(first_run: bool = False):
    ...
    _update_last_f1(eval_summaries)
  ...
  def _run_inference_eval():
    ...
    _update_last_f1(all_metrics_done)
  ...
  for epoch in range(first_epoch, num_epochs):
    ...
    progress_bar = _create_epoch_progress_bar(
        epoch - first_epoch + 1,
        num_epochs - first_epoch,
        max(epoch_end_step - host_step, 0),
    )
    if progress_bar is not None:
      progress_bar.set_postfix(
          _build_progress_postfix(last_train_loss, last_f1_value, last_f1_name)
      )
    ...
    try:
      ...
      while host_step < epoch_end_step:
        ...
        if progress_bar is not None:
          progress_bar.update(inner_num_steps)

        train_summary_metrics = None
        if progress_bar is not None or not concurrent_metrics:
          train_summary_metrics = train_summary.result()
        if train_summary_metrics is not None:
          flat_train_metrics = _flatten_metric_mapping(train_summary_metrics)
          _, last_train_loss = _select_metric_value(
              flat_train_metrics, _LOSS_CANDIDATE_KEYS
          )
        if progress_bar is not None:
          progress_bar.set_postfix(
              _build_progress_postfix(
                  last_train_loss, last_f1_value, last_f1_name
              )
          )
        if not concurrent_metrics:
          ...
          trainer.stop_training = run_actions(  # pytype: disable=wrong-arg-types  # jax-ndarray
              ...
              actions,
              trainer.train_state,
              {TRAIN_METRIC_KEY: train_summary_metrics},
          )
