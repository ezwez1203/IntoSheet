...
try:
  from jestimator import amos as jestimator_amos
  from jestimator import amos_helper
  _HAS_JESTIMATOR = True
except ImportError:
  _HAS_JESTIMATOR = False
import optax
...
class OptaxStatePartitionRules:
  ...
  # Build _RULES dynamically to handle different optax versions gracefully.
  _RULES = {}

  # -- Jestimator AMOS --
  if _HAS_JESTIMATOR:
    _RULES[jestimator_amos.ScaleByAmosState] = amos_helper.state_partition_rule

  # -- Leaf Optax States --
  if hasattr(optax, 'AddNoiseState'):
    _RULES[optax.AddNoiseState] = lambda state, params_axes: optax.AddNoiseState(count=None, rng_key=None)
  if hasattr(optax, 'EmaState'):
    _RULES[optax.EmaState] = lambda state, params_axes: optax.EmaState(
        count=None,
        ema=OptaxStatePartitionRules.derive_params_axes(state.ema, params_axes))
  if hasattr(optax, 'EmptyState'):
    _RULES[optax.EmptyState] = lambda state, params_axes: optax.EmptyState()
  if hasattr(optax, 'TraceState'):
    _RULES[optax.TraceState] = lambda state, params_axes: optax.TraceState(
        trace=OptaxStatePartitionRules.derive_params_axes(state.trace, params_axes))
  if hasattr(optax, 'ScaleByAdamState'):
    _RULES[optax.ScaleByAdamState] = lambda state, params_axes: optax.ScaleByAdamState(
        count=None,
  ...
  if hasattr(optax, 'ScaleByBeliefState'):
    _RULES[optax.ScaleByBeliefState] = lambda state, params_axes: optax.ScaleByBeliefState(
        count=None,
        mu=OptaxStatePartitionRules.derive_params_axes(state.mu, params_axes),
        nu=OptaxStatePartitionRules.derive_params_axes(state.nu, params_axes))
  if hasattr(optax, 'ScaleByLionState'):
    _RULES[optax.ScaleByLionState] = lambda state, params_axes: optax.ScaleByLionState(
        count=None,
        mu=OptaxStatePartitionRules.derive_params_axes(state.mu, params_axes))
  if hasattr(optax, 'ScaleByRssState'):
    _RULES[optax.ScaleByRssState] = lambda state, params_axes: optax.ScaleByRssState(
        sum_of_squares=OptaxStatePartitionRules.derive_params_axes(state.sum_of_squares, params_axes))
  if hasattr(optax, 'ScaleByRmsState'):
    _RULES[optax.ScaleByRmsState] = lambda state, params_axes: optax.ScaleByRmsState(
        nu=OptaxStatePartitionRules.derive_params_axes(state.nu, params_axes))
  if hasattr(optax, 'ScaleByRStdDevState'):
    _RULES[optax.ScaleByRStdDevState] = lambda state, params_axes: optax.ScaleByRStdDevState(
        mu=OptaxStatePartitionRules.derive_params_axes(state.mu, params_axes),
        nu=OptaxStatePartitionRules.derive_params_axes(state.nu, params_axes))
  if hasattr(optax, 'ScaleBySM3State'):
    _RULES[optax.ScaleBySM3State] = lambda state, params_axes: optax.ScaleBySM3State(
        mu=OptaxStatePartitionRules.derive_params_axes(state.mu, params_axes),
        nu=OptaxStatePartitionRules.derive_params_axes(state.nu, params_axes))
  if hasattr(optax, 'ScaleByTrustRatioState'):
    _RULES[optax.ScaleByTrustRatioState] = lambda state, params_axes: optax.ScaleByTrustRatioState()
  if hasattr(optax, 'ScaleByScheduleState'):
    _RULES[optax.ScaleByScheduleState] = _scale_by_schedule_ctor
  if hasattr(optax, 'ZeroNansState'):
    _RULES[optax.ZeroNansState] = lambda state, params_axes: optax.ZeroNansState(found_nan=None)

  # -- Recursive / Combinator Optax States --
  if hasattr(optax, 'MaskedState'):
    _RULES[optax.MaskedState] = lambda state, params_axes: optax.MaskedState(
        inner_state=OptaxStatePartitionRules.derive_optax_logical_axes(state.inner_state, params_axes))
  if hasattr(optax, 'InjectHyperparamsState'):
    _RULES[optax.InjectHyperparamsState] = lambda state, params_axes: optax.InjectHyperparamsState(
        count=None,
        hyperparams=jax.tree.map(lambda x: None, state.hyperparams),
        inner_state=OptaxStatePartitionRules.derive_optax_logical_axes(state.inner_state, params_axes))
  if hasattr(optax, 'MultiStepsState'):
    _RULES[optax.MultiStepsState] = lambda state, params_axes: optax.MultiStepsState(
        mini_step=None, gradient_step=None,
        inner_opt_state=OptaxStatePartitionRules.derive_optax_logical_axes(state.inner_opt_state, params_axes),
        acc_grads=params_axes)
  if hasattr(optax, 'ApplyIfFiniteState'):
    _RULES[optax.ApplyIfFiniteState] = lambda state, params_axes: optax.ApplyIfFiniteState(
        notfinite_count=None, last_finite=None, total_notfinite=None,
        inner_state=OptaxStatePartitionRules.derive_optax_logical_axes(state.inner_state, params_axes))
  if hasattr(optax, 'ConditionallyTransformState'):
    _RULES[optax.ConditionallyTransformState] = lambda state, params_axes: optax.ConditionallyTransformState(
        inner_state=OptaxStatePartitionRules.derive_optax_logical_axes(state.inner_state, params_axes),
        step=None)
  if hasattr(optax, 'MultiTransformState'):
    _RULES[optax.MultiTransformState] = lambda state, params_axes: optax.MultiTransformState(
        inner_states=OptaxStatePartitionRules.derive_optax_logical_axes(state.inner_states, params_axes))
...
if _HAS_JESTIMATOR:
  amos = wrap_optax_optimizer(jestimator_amos.amos)
