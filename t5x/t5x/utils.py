...
try:
  ...
  class _AirioStub:
    """Fallback stub when optional `airio` is not installed."""
    ...
    class AirIODatasetIterator:
      pass

    class dataset_providers:
      """Namespace for API compatibility."""

      @staticmethod
      def get_vocabularies(_):
        raise ImportError(
            'airio is not installed. Install optional airio dependencies '
            'to use AirIO dataset providers.'
        )
...
# JAX 0.4.x may not expose `jax.tree`; T5X code expects `jax.tree.map/leaves`.
