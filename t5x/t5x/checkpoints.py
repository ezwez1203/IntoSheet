...
def _register_dataset_args(handler_cls):
  register_sig = inspect.signature(ocp.args.register_with_handler)
  if 'for_restore' in register_sig.parameters:
    return ocp.args.register_with_handler(
        handler_cls, for_save=True, for_restore=True
    )
  return ocp.args.register_with_handler(handler_cls, for_save=True)


@_register_dataset_args(DatasetCheckpointHandler)
@dataclasses.dataclass
...
class OrbaxCheckpointManagerInterface:
  ...
  def __init__(
    ...
    # `AsyncOptions` and `async_options` were introduced in newer Orbax.
    if hasattr(ocp, 'AsyncOptions') and (
        'async_options' in inspect.signature(ocp.CheckpointManagerOptions).parameters
    ):
      options_kwargs['async_options'] = ocp.AsyncOptions(timeout_secs=600)
