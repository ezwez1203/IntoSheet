...
class PjitPartitioner(BasePjitPartitioner):
  ...
  def __init__(
    ...
    # Older JAX releases (e.g. 0.4.23) do not expose this config knob.
    self._use_shardy_partitioner = getattr(
        jax.config, 'jax_use_shardy_partitioner', False
    )
  ...
  def partition(
    ...
    if (
        getattr(jax.config, 'jax_use_shardy_partitioner', False)
        != self._use_shardy_partitioner
    ):
      raise ValueError(
